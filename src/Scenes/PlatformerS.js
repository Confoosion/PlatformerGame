class PlatformerS extends Phaser.Scene {
    ...
    preload()
    {
        this.load.image("movingTile", "assets/tile_bridge.png");
    }
    ...
    create() {
        this.map = this.add.tilemap("level-S-platform", 64, 64, 40, 20);
        this.tileset = this.map.addTilesetImage("level-S", "tiles");
        this.objectTileset = this.map.addTilesetImage("level-S-Obby", "tiles");
        this.groundLayer = this.map.createLayer("floor-and-tiles", this.tileset, 0, 0);
        // set up player avatar
        my.sprite.player = this.physics.add.sprite(2500, 1100, "characterS").setScale(0.5);
        ...
        for(let i = 0; i < this.moveCoords.length; i += 4)
        {
            ...
            my.sprite.movingTile = this.add.follower(this.curve, this.moveTileCoords[i*0.5], this.moveTileCoords[i*0.5+1], "movingTile");
            my.sprite.movingTile.startFollow ({
                from: 0,
                to: 1,
                delay: 0,
                duration: 3000,
            ...
            let moveTile = this.physics.add.existing(my.sprite.movingTile);
            this.physics.add.collider(my.sprite.player, moveTile);
            moveTile.body.immovable = true;
            // this.physics.enable([this.moveTile], Phaser.Physics.ARCADE);
            moveTile.body.setAllowGravity(false);
            movingTileGroup[i*0.25] = moveTile;
        }
